...
class MvDesc: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate, UISearchBarDelegate {
    ...
    @IBOutlet weak var plays: UIButton!
    ...
    override func viewDidLoad() {
        ...
        if(viewingExistingMov){
            ...
            isVideoPresent = findIfVideoPresent(movDesc["title"]!)
            if(isVideoPresent){
               plays.hidden = false
            }
            else{
    ...
    func findIfVideoPresent(title:String) -> Bool{
        let fetchRequest = NSFetchRequest(entityName: "Movies")
        //Searched with the predicate of the movie
        fetchRequest.predicate = NSPredicate(format: "title = %@", movTitle.text!)
        //
        do{
            let movieList = try mContext!.executeFetchRequest(fetchRequest)
            let movie = movieList[0]
            let videoFileName = movie.valueForKey("videoFilename")
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
             if segue.identifier == "playMovie"{
                ...
                let navigationController = segue.destinationViewController as! PlayMovieController
                let fetchRequest = NSFetchRequest(entityName: "Movies")
                //Searched with the predicate of the movie
                fetchRequest.predicate = NSPredicate(format: "title = %@", movTitle.text!)
                //
                do{
                    let movieList = try mContext!.executeFetchRequest(fetchRequest)
                    let movie = movieList[0]
                    let videoFileName = movie.valueForKey("videoFilename")! as! String
                    
                    //Set the value to segue variable declared in next class
                    navigationController.file = videoFileName
